              ...
              >
                ...
                >
                  {[
                    ...
                    <motion.div key={s.title} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.7 + i * 0.1, duration: 0.3 }} className={`p-3 sm:p-4 rounded-lg border ${s.bgColor} ${s.borderColor}`}>
                      <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                        <s.icon className={`w-4 h-4 sm:w-5 sm:h-5 ${s.textColor}`} />
                        <h4 className={`font-semibold text-xs sm:text-sm ${s.textColor}`}>{s.title}</h4>
                      </div>
                      <p className="text-text/90 text-justify text-xs sm:text-sm">{s.content}</p>
                    </motion.div>
                  ))}
                </motion.div>

                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.1, duration: 0.4 }} className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 mt-4 sm:mt-6">
                  {activeProject.githubUrl && (
                    <motion.a whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }} href={activeProject.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-lg border border-accent text-accent hover:bg-accent hover:text-white transition-all duration-300 text-sm sm:text-base">
                      <ExternalLink size={14} /> GitHub
                    </motion.a>
                  )}
                  <motion.button whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }} onClick={() => setActiveProject(null)} className="inline-flex items-center justify-center px-3 sm:px-4 py-2 rounded-lg bg-accent text-white hover:opacity-90 transition-all duration-300 text-sm sm:text-base">
                    Close
                  </motion.button>
                </motion.div>
              </motion.div>
